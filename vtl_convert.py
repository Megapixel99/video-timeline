...
def describe_camera(vs: VisualSignals, a: int, b: int) -> tuple[str, str]:
    ...
    ev = (f"pan {pan:+.3f} framewidths/s, tilt {tilt:+.3f} frameheights/s, "
          ...
          f"steadiness {k_pan:.1f}/{k_tilt:.1f}/{k_zoom:.1f} (pan/tilt/zoom, "
          f"median over scatter; below 0.5 is not a coherent move)")
...
def render_markdown(b: dict) -> str:
    ...
    A(f"`{FORMAT_VERSION}` · generated by vtl_convert {CONVERTER_VERSION} · "
      f"{summ['duration_human']} · {summ['shot_count']} shots in {summ['scene_count']} scenes "
    ...
    for s in b["shots"]:
        ...
        if sc and sc is not prev_scene:
    ...
    if b.get("sheets"):
        A(f"- `sheets/` — {len(b['sheets'])} contact sheets, one per scene, in time order")
    A("")
    return "\n".join(o)
